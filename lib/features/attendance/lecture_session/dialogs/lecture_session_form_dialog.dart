...
class LectureSessionFormDialog extends StatefulWidget {
  const LectureSessionFormDialog({
    super.key,
  ...
  @override
  State<LectureSessionFormDialog> createState() =>
      _LectureSessionFormDialogState();
}

class _LectureSessionFormDialogState extends State<LectureSessionFormDialog> {
  ...
  void initState() {
    ...
    _dateController = TextEditingController(text: _formatDate(_lectureDate));
  ...
  Future<void> _save() async {
    ...
    if (widget.isEditing) {
      success = await provider.updateLectureSession(lectureSession);
    } else {
      success = await provider.createLectureSession(lectureSession);
    }
    ...
    setState(() {
      _generalError =
          ...
          (widget.isEditing
              ? 'Unable to update lecture session.'
              : 'Unable to create lecture session.');
    });
  }
  ...
  void _changeFromTime(String? value) {
    if (value == null) return;

    setState(() {
      _fromTime = value;

      if (_timeToMinutes(_toTime) <= _timeToMinutes(_fromTime)) {
        final fromIndex = _timeOptions.indexOf(_fromTime);

        if (fromIndex >= 0 && fromIndex + 1 < _timeOptions.length) {
  ...
  void _changeToTime(String? value) {
    if (value == null) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<LectureSessionProvider>(
      builder: (context, provider, child) {
        final isEditing = widget.isEditing;

        return AppFormDialog(
          title: isEditing ? 'Edit Lecture Session' : 'Create Lecture Session',
          formKey: _formKey,
          isSaving: provider.isLoading,
          errorMessage: _generalError,
          saveButtonText: isEditing ? 'Update' : 'Create',
          savingButtonText: isEditing ? 'Updating...' : 'Creating...',
          saveIcon: isEditing ? Icons.edit_rounded : Icons.add_rounded,
          onSave: _save,
          children: [
            _buildDateField(context, r, provider),

            SizedBox(height: r.spacingM),

            _buildWeekField(context, r, provider),
  ...
  Widget _buildDateField(
    ...
    AppResponsive r,
    LectureSessionProvider provider,
  ) {
    ...
    return DropdownButtonFormField<String>(
      ...
      items: _timeOptions.map((time) {
        return DropdownMenuItem<String>(
          value: time,
          child: Text(time, style: TextStyle(fontSize: r.body)),
        );
      }).toList(),
